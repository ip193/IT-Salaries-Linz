...
exp_pay_truncated_males_x = np.delete(exp_pay_truncated_males_x, experienced_males)  #  remove very experienced men from the set
...
truncated_all_x = np.concatenate((np.array([all_list[i][1] for i in range(len(all_list))]), truncated_males_x, females_x))

truncated_all_y = np.concatenate(((np.array([all_list[i][0] for i in range(len(all_list))]), truncated_males_y, females_y)))
...
plt.plot(xp, truncated_all_p1(xp), 'k--', label='Erwartetes Gehalt, r = ' + str(int(np.corrcoef(truncated_all_x, truncated_all_y)[0, 1]*100)/100))
